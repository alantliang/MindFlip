...
public class Astar {
    ...
    init(graph: Graph, start: (Int, Int), goal: (Int, Int)) {
        ...
        openSet = [self.start]
        closedSet = []  // do these need to be sets instead of lists?
    ...
    func getMinHeuristicCostFromStartNode() -> Node {
        ...
        var returnNode: Node?
        println(openSet.count)
        for node in openSet {
            println("Hi")
            println(heuristicCostFromStart[node])
...
var x = Graph(width: 3, height: 3)

var y = Astar(graph: x, start: (1, 1), goal: (2, 2))
var z = y.run()
